...
<div class="container-fluid px-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 mt-4">
        ...
        <a href="{{ route('admin.diadiem.create') }}" class="btn btn-danger btn-lg shadow-sm">
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1 ms-3">
                            ...
                            <h3 class="mb-0 fw-bold">{{ $diaDiems->total() }}</h3>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1 ms-3">
                            ...
                            <h3 class="mb-0 fw-bold text-success">{{ $diaDiems->where('TrangThai', 'KhaDung')->count() }}</h3>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1 ms-3">
                            ...
                            <h3 class="mb-0 fw-bold text-warning">{{ $diaDiems->where('TrangThai', '!=', 'KhaDung')->count() }}</h3>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1 ms-3">
                            ...
                            <h3 class="mb-0 fw-bold text-info">{{ number_format($diaDiems->avg('GiaTien') ?? 0, 0, ',', '.') }}</h3>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @forelse ($diaDiems as $index => $item)
                            <tr>
                                <td class="text-center fw-semibold text-muted">
                                    {{ ($diaDiems->currentPage() - 1) * $diaDiems->perPage() + $index + 1 }}
                                </td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <span class="fw-semibold text-dark">{{ $item->TenDiaDiem }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <span class="text-muted">
                                        <i class="fas fa-location-dot me-1 text-primary"></i>{{ Str::limit($item->DiaChi, 50) }}
                                    </span>
                                </td>
                                <td class="text-end">
                                    <span class="badge bg-success-subtle text-success fs-6 fw-semibold px-3 py-2">
                                        {{ number_format($item->GiaTien, 0, ',', '.') }} ₫
                                    </span>
                                </td>
                                <td class="text-center">
                                    @if ($item->TrangThai == 'KhaDung')
                                ...
                                <td class="text-center">
                                    <div class="btn-group" role="group">
                                        <a href="{{ route('admin.diadiem.edit', $item->id) }}" 
                                           class="btn btn-warning btn-sm" 
                                           title="Chỉnh sửa"
                                           data-bs-toggle="tooltip">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('admin.diadiem.destroy', $item->id) }}" 
                                              method="POST" 
                                              class="d-inline"
                                              onsubmit="return confirm('Bạn có chắc chắn muốn xóa địa điểm \'{{ $item->TenDiaDiem }}\' không?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-5">
                                    <div class="text-muted">
                                        ...
                                        <a href="{{ route('admin.diadiem.create') }}" class="btn btn-danger">
        ...
        <div class="card-footer bg-white border-top">
            <div class="d-flex justify-content-between align-items-center">
                <div class="text-muted small">
                    Hiển thị {{ $diaDiems->firstItem() ?? 0 }} - {{ $diaDiems->lastItem() ?? 0 }} 
                    trong tổng số {{ $diaDiems->total() }} địa điểm
                </div>
                <div>
                    {{ $diaDiems->links() }}
                </div>
