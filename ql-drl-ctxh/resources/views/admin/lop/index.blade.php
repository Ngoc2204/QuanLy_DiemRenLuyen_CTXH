...
<div class="container-fluid">
    ...
    <div class="stats-grid">
        <div class="stat-card">
            <div class="stat-card-content">
                ...
                <div class="stat-info">
                    <h6>Tổng số lớp</h6>
                    {{-- Giả định biến $total truyền từ Controller --}}
                    <div class="stat-value">{{ $total ?? 0 }}</div> 
                </div>
            </div>
        </div>
        
        {{-- Có thể thêm thống kê khác như: Lớp có nhiều sinh viên nhất, Tổng số khoa liên quan... --}}
        {{-- Ví dụ:
        <div class="stat-card">
            <div class="stat-card-content">
                <div class="stat-icon" style="background: linear-gradient(135deg, var(--success), #059669); box-shadow: 0 8px 16px -4px rgba(16, 185, 129, 0.4);">
                    <i class="fa-solid fa-building-columns"></i>
                </div>
                <div class="stat-info">
                    <h6>Số khoa đang quản lý</h6>
    ...
    <div class="main-card">
        <div class="card-header-modern">
            ...
            <a href="{{ route('admin.lop.create') }}" class="btn-modern btn-primary-modern">
        ...
        <div class="search-section">
            <form method="GET" action="{{ route('admin.lop.index') }}" class="search-form">
                ...
                {{-- Select Lọc theo Khoa (Kế thừa từ code cũ của bạn) --}}
                <div class="search-select-wrapper">
                    <select name="MaKhoa" class="form-select-modern">
                        <option value="">-- Tất cả khoa --</option>
                        {{-- Giả định biến $khoas được truyền từ Controller --}}
                        @foreach ($khoas as $k) 
                ...
                {{-- Button Đặt lại --}}
                @if(request()->filled('keyword') || request()->filled('MaKhoa'))
                <a href="{{ route('admin.lop.index') }}" class="btn-reset">
        ...
        <div class="table-responsive">
            <table class="table-modern">
                ...
                <tbody>
                    {{-- Giả định biến $lops là một Paginator/Collection chứa dữ liệu lớp --}}
                    ...
                    <tr>
                        ...
                        <td class="fw-semibold">{{ $lop->TenLop }}</td>
                        <td>
                             <span class="badge-info">
                                {{ $lop->khoa->TenKhoa ?? 'Chưa xác định' }}
                             </span>
                        </td>
                        <td>
                            <div class="action-buttons">
                                <a href="{{ route('admin.lop.edit', $lop->MaLop) }}" 
                                   class="btn-action-modern btn-edit" 
                                   title="Chỉnh sửa">
                                    <i class="fa-solid fa-pen"></i>
                                </a>
                                <form action="{{ route('admin.lop.destroy', $lop->MaLop) }}" 
                                      method="POST" 
                                      class="d-inline"
                                      onsubmit="return confirm('Bạn có chắc muốn xóa lớp {{ $lop->TenLop }} này không?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                    ...
                    <tr>
                        <td colspan="5">
                            <div class="empty-state">
                                ...
                                <p>Không có lớp nào được tìm thấy theo tiêu chí của bạn. Hãy thử tìm kiếm khác hoặc <a href="{{ route('admin.lop.create') }}">thêm lớp mới</a>.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="pagination-wrapper">
            {{ $lops->appends(request()->query())->links('pagination::bootstrap-5') }}
        </div>
        @endif
    </div>
</div>
