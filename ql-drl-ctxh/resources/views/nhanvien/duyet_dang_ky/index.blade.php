...
@section('title', 'Duyệt Đăng ký Hoạt động')
@section('page_title', 'Duyệt Đăng ký Sinh viên')
...
<div class="container-fluid px-4">
    ...
    <div class="card modern-card mb-4">
        <div class="card-body p-4">
            <form method="GET" action="{{ route('nhanvien.duyet_dang_ky.index') }}">
                <div class="row g-3 align-items-end">
                    <div class="col-lg-4 col-md-6">
                        ...
                        <select name="type" id="type" class="form-select modern-select">
                            ...
                            <option value="DRL" {{ request('type') == 'DRL' ? 'selected' : '' }}>
                                <i class="fa-solid fa-star"></i> Điểm Rèn Luyện (DRL)
                            </option>
                            <option value="CTXH" {{ request('type') == 'CTXH' ? 'selected' : '' }}>
                    ...
                    <div class="col-lg-4 col-md-6">
                        ...
                        <input type="date" name="date" id="date" class="form-control modern-select" value="{{ request('date') }}">
                    </div>
                    ...
                    <div class="col-lg-2 col-md-3 col-6">
                        <a href="{{ route('nhanvien.duyet_dang_ky.index') }}" class="btn btn-outline-secondary w-100 modern-btn">
    ...
    @if(request()->has('date') && request('date') != '' && $dangKys->count() > 0)
    <div class="card modern-card mb-4">
        <div class="card-body p-4 bg-light-subtle">
            <form action="{{ route('nhanvien.duyet_dang_ky.batch_approve') }}" method="POST"
                onsubmit="return confirm('Bạn có chắc chắn muốn DUYỆT TẤT CẢ {{ $dangKys->total() }} đăng ký cho ngày {{ request('date') }}?')">
                @csrf
                <input type="hidden" name="approve_date" value="{{ request('date') }}">
                <input type="hidden" name="approve_type" value="{{ request('type') }}">
                <div class="d-flex justify-content-between align-items-center">
                    <div class="fw-semibold text-success">
                        <i class="fa-solid fa-calendar-check me-2"></i>
                        Tìm thấy <span class="badge bg-success rounded-pill">{{ $dangKys->total() }}</span> đăng ký chờ duyệt cho ngày {{ \Carbon\Carbon::parse(request('date'))->format('d/m/Y') }}.
                    </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table modern-table mb-0">
                    ...
                    <tbody>
                        @forelse ($dangKys as $index => $dk)
                        @php
                        $hoatDong = $dk->hoatDongRel;
                        @endphp
                        <tr class="table-row-hover">
                            <td class="text-center">
                                <span class="badge-number">{{ $dangKys->firstItem() + $index }}</span>
                            </td>
                            <td>
                                @if($dk->sinhVien)
                                <div class="d-flex align-items-center">
                                    <div class="avatar-circle">
                                        <span>{{ strtoupper(substr($dk->sinhVien->HoTen, 0, 1)) }}</span>
                                    </div>
                                    <div class="ms-3">
                                        <div class="student-name">{{ $dk->sinhVien->HoTen }}</div>
                                        <div class="student-code">{{ $dk->MSSV }}</div>
                                    </div>
                                </div>
                                @else
                                <span class="text-danger fw-semibold">
                                    <i class="fa-solid fa-exclamation-triangle me-1"></i>{{ $dk->MSSV }} (Lỗi)
                                </span>
                                @endif
                            </td>
                            <td>
                                @if($hoatDong)
                                {{-- Hiển thị tên hoạt động --}}
                                <a href="{{ $dk->type == 'DRL' ? route('nhanvien.hoatdong_drl.show', $hoatDong->MaHoatDong) : route('nhanvien.hoatdong_ctxh.show', $hoatDong->MaHoatDong) }}"
                                    target="_blank"
                                    class="activity-link">
                                    {{ Str::limit($hoatDong->TenHoatDong, 50) }}
                                    <i class="fa-solid fa-external-link ms-1"></i>
                                </a>
                                <div class="activity-code">{{ $hoatDong->MaHoatDong }}</div>
                                @else
                                {{-- "Không tìm thấy" sẽ hiển thị ở đây nếu $dk->hoatDongRel bị null --}}
                                <span class="text-danger fw-semibold">
                            ...
                            <td class="text-center">
                                <div class="date-display">
                                    ...
                                    {{ optional($dk->NgayDangKy)->format('d/m/Y') }}
                                </div>
                                <div class="time-display">
                                    <i class="fa-regular fa-clock me-1"></i>
                                    {{ optional($dk->NgayDangKy)->format('H:i') }}
                                </div>
                            </td>
                            <td class="text-center">
                                <div class="action-buttons">
                                    {{-- Nút Duyệt --}}
                                    <form action="{{ route('nhanvien.duyet_dang_ky.update', ['duyet_dang_ky' => $dk->MaDangKy, 'type' => $dk->type]) }}"
                                        method="POST"
            ...
            <div class="pagination-wrapper">
                <div class="pagination-info">
                    ...
                    Hiển thị <strong>{{ $dangKys->firstItem() }}</strong> - <strong>{{ $dangKys->lastItem() }}</strong>
                    trong tổng số <strong>{{ $dangKys->total() }}</strong> kết quả
                </div>
                <div class="pagination-links">
                    {{ $dangKys->appends(request()->query())->links() }}
                </div>
            </div>
            @endif
        </div>
