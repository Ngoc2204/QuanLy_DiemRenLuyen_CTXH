...
@section('title', 'Quản lý Thanh toán (Chờ duyệt)')
...
<div class="container-fluid py-4">
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-6">
            <div class="stats-card pending">
                ...
                <h3 class="mb-0">{{ $thanhToans->total() }} giao dịch</h3>
            </div>
        </div>
        <div class="col-md-6">
            <div class="stats-card total">
                ...
                <h3 class="mb-0">{{ number_format($thanhToans->sum('TongTien'), 0, ',', '.') }} đ</h3>
            </div>
        </div>
    </div>
    ...
    <div class="search-card mb-4">
        ...
        <form action="{{ route('nhanvien.thanhtoan.index') }}" method="GET" class="row g-3">
            <div class="col-md-5">
                <label for="search_mssv" class="form-label">
                    <i class="fas fa-search me-1"></i>Tìm theo MSSV
                </label>
                <input type="text" class="form-control" id="search_mssv" name="search_mssv" value="{{ request('search_mssv') }}" placeholder="Nhập MSSV sinh viên...">
            </div>
            <div class="col-md-4">
                ...
                <select class="form-select" id="phuong_thuc" name="phuong_thuc">
                    ...
                    <option value="TienMat" {{ request('phuong_thuc') == 'TienMat' ? 'selected' : '' }}>Tiền mặt</option>
                    <option value="Online" {{ request('phuong_thuc') == 'Online' ? 'selected' : '' }}>Chuyển khoản Online</option>
                </select>
            </div>
            <div class="col-md-3">
    ...
    <div class="data-table-card">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <div class="id-group">
                                <span class="id-badge primary">HD-{{ $tt->id }}</span>
                                @if($donDangKy)
                                <span class="id-badge secondary">DK-{{ $donDangKy->MaDangKy }}</span>
                                @endif
                            </div>
                        </td>
                        <td>
                        ...
                        <td>
                            @if($hoatDong)
                            <div class="fw-semibold text-dark">{{ $hoatDong->TenHoatDong }}</div>
                            @else
                            <span class="text-muted fst-italic">Hoạt động đã bị xóa</span>
                            @endif
                        </td>
                        <td>
                            <div class="amount-display">{{ number_format($tt->TongTien, 0, ',', '.') }} đ</div>
                        </td>
                        <td>
                            @if ($tt->PhuongThuc == 'TienMat')
                            <span class="badge badge-cash">
                                <i class="fas fa-money-bill-wave me-1"></i>Tiền mặt
                            </span>
                            @elseif ($tt->PhuongThuc == 'Online')
                        ...
                        <td>
                            @if($donDangKy)
                            <div class="badge-id">DK{{ $donDangKy->MaDangKy }}</div>
                            @else
                            <span class="text-muted">-</span>
                            @endif
                        </td>
                        <td class="text-center">
                            <button type="button" class="btn btn-success btn-action" data-bs-toggle="modal" data-bs-target="#confirmModal-{{ $tt->id }}">
                    ...
                    {{-- Modal Xác nhận --}}
                    <div class="modal fade" id="confirmModal-{{ $tt->id }}" tabindex="-1" aria-labelledby="modalLabel-{{ $tt->id }}" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="modalLabel-{{ $tt->id }}">
                                        <i class="fas fa-check-circle text-success me-2"></i>Xác nhận thanh toán
                                    </h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <form action="{{ route('nhanvien.thanhtoan.xacnhan', $tt->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <div class="modal-body">
                                        ...
                                        <ul class="list-group list-group-flush mb-3">
                                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                                ...
                                                <strong>{{ $sinhVien?->HoTen }} ({{ $sinhVien?->MSSV }})</strong>
                                            </li>
                                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                                <span class="text-muted">
                                                    <i class="fas fa-calendar-alt me-2"></i>Hoạt động
                                                </span>
                                                <strong>{{ $hoatDong?->TenHoatDong ?? 'N/A' }}</strong>
                                            </li>
                                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                                <span class="text-muted">
                                                    <i class="fas fa-money-bill-wave me-2"></i>Số tiền
                                                </span>
                                                <strong class="text-danger fs-5">{{ number_format($tt->TongTien, 0, ',', '.') }} đ</strong>
                                            </li>
                                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                                <span class="text-muted">
                                        ...
                                        @if ($tt->PhuongThuc == 'Online')
                                        <div class="mb-3">
                                            <label for="ma_giao_dich-{{ $tt->id }}" class="form-label">
                                                <i class="fas fa-hashtag me-1"></i>Mã giao dịch ngân hàng
                                            </label>
                                            <input type="text" class="form-control" name="ma_giao_dich" id="ma_giao_dich-{{ $tt->id }}" placeholder="Nhập mã giao dịch (nếu có)...">
        ...
        <div class="p-3 border-top">
            {{ $thanhToans->appends(request()->query())->links() }}
        </div>
        @endif
    </div>
</div>
