...
<div class="schedule-wrapper">
    <div class="schedule-container">
        ...
        <div class="schedule-header">
            ...
            <div class="week-info">Tuần: {{ $startOfWeek->format('d/m') }} - {{ $endOfWeek->format('d/m/Y') }}</div>
        </div>
        ...
        <div class="schedule-grid">
            <!-- Time Column -->
            <div class="time-col">
                {{-- Thêm style để gán Cột 1 và Hàng 1-4 cho các ca --}}
            ...
            {{-- Dùng $loop->index (bắt đầu từ 0) + 2 để gán cột (cột 1 là 'Ca học') --}}
            <div class="day-col">
                <div class="day-header {{ $dayData['date']->isToday() ? 'today' : '' }}" style="grid-row: 1; grid-column: {{ $loop->index + 2 }};">
                ...
                {{-- Ca Sáng --}}
                <div class="class-cell" style="grid-row: 2; grid-column: {{ $loop->index + 2 }};">
                    @foreach($morning as $item)
                    @php($activity = $item['hoatdong'])
                    <div class="class-card {{ $item['type'] == 'DRL' ? 'theory' : 'practice' }} {{ $dayData['date']->isToday() && $item['trang_thai'] == 'Đã duyệt' && $activity->CheckInToken ? 'has-actions' : '' }}">
                        <span class="class-type-badge">{{ $item['type'] }}</span>
                        <div class="class-name">{{ $activity->TenHoatDong }}</div>
                        <div class="class-code">{{ $item['trang_thai'] ?? 'Đang diễn ra' }}</div>
                        <div class="class-time">
                            <i class="fas fa-clock"></i>
                            {{ $activity->ThoiGianBatDau->format('H:i') }} - {{ $activity->ThoiGianKetThuc->format('H:i') }}
                        </div>
                        <div class="class-location">
                            <i class="fas fa-map-marker-alt"></i>
                            {{ $activity->DiaDiem ?? 'Chưa xác định' }}
                        </div>
                        @if($dayData['date']->isToday() && $item['trang_thai'] == 'Đã duyệt' && $activity->CheckInToken)
                        <div class="class-actions">
                            <a href="{{ route('sinhvien.scan', $activity->CheckInToken ?? '#') }}" class="btn-scan check-in">Check In</a>
                            <a href="{{ route('sinhvien.scan', $activity->CheckOutToken ?? '#') }}" class="btn-scan check-out">Check Out</a>
                        </div>
                        @endif
                    </div>
                    @endforeach
                ...
                {{-- Ca Chiều --}}
                <div class="class-cell" style="grid-row: 3; grid-column: {{ $loop->index + 2 }};">
                    @foreach($afternoon as $item)
                    @php($activity = $item['hoatdong'])
                    <div class="class-card {{ $item['type'] == 'DRL' ? 'theory' : 'practice' }} {{ $dayData['date']->isToday() && $item['trang_thai'] == 'Đã duyệt' && $activity->CheckInToken ? 'has-actions' : '' }}">
                        <span class="class-type-badge">{{ $item['type'] }}</span>
                        <div class="class-name">{{ $activity->TenHoatDong }}</div>
                ...
                {{-- Ca Tối --}}
                <div class="class-cell" style="grid-row: 4; grid-column: {{ $loop->index + 2 }};">
