...
<div class="container-fluid">
    <div class="payment-card">
        <div class="payment-header">
            ...
            {{-- SỬA LỖI 1: $thanhToan->SoTien -> $thanhToan->TongTien --}}
            <h4 class="payment-amount">{{ number_format($thanhToan->TongTien, 0, ',', '.') }}<sup>đ</sup></h4>
        </div>
        ...
        <ul class="info-list">
            <li>
                ...
                {{-- Lấy qua quan hệ 1-1 --}}
                <span class="value">{{ $hoatDong->TenHoatDong ?? 'Hoạt động Địa chỉ đỏ' }}</span>
            </li>
            <li>
                <span class="label">Địa điểm:</span>
                <span class="value">{{ $hoatDong->diaDiem->TenDiaDiem ?? 'N/A' }}</span>
            </li>
            <li>
                <span class="label">Mã hóa đơn:</span>
                <span class="value">HD-{{ str_pad($thanhToan->id, 6, '0', STR_PAD_LEFT) }}</span>
            </li>
            <li>
                <span class="label">Mã Đăng Ký:</span>
                
                {{-- SỬA LỖI 2: $thanhToan->MaDangKy -> $thanhToan->dangKyHoatDong->MaDangKy --}}
                {{-- (Vì MaDangKy nằm trên đơn đăng ký, không phải trên hóa đơn) --}}
                <span class="value">DK-{{ str_pad($thanhToan->dangKyHoatDong->MaDangKy, 6, '0', STR_PAD_LEFT) }}</span>
            </li>
        </ul>
        ...
        <div class="method-wrapper">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    <form action="{{ route('sinhvien.thanhtoan.chon_phuong_thuc', $thanhToan->id) }}" method="POST">
                        @csrf
