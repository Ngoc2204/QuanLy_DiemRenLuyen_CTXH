...
<div class="container-fluid" style="max-width: 1400px; margin: 0 auto; padding: 2rem 1rem;">
        ...
        <ul class="nav custom-tabs" id="activityTab" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="ctxh-tab" data-bs-toggle="tab" data-bs-target="#ctxh-content" type="button" role="tab" aria-controls="ctxh-content" aria-selected="false">
                    ...
                    <span class="badge bg-light text-dark">{{ $activitiesCTXH->count() }}</span>
                </button>
            </li>
        </ul>
    </div>
    ...
    <div class="tab-content" id="activityTabContent">
            ...
            <div class="activity-card drl-card">
                <div class="activity-header">
                    <span class="activity-type drl">
                        <i class="fas fa-star"></i>
                        {{ $activity->LoaiHoatDong ?? 'Rèn luyện' }}
                    </span>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <p class="activity-meta">
                                ...
                                <span>Bắt đầu: <strong>{{ $activity->ThoiGianBatDau->format('d/m/Y H:i') }}</strong></span>
                            </p>
                            <p class="activity-meta">
                        ...
                        <div class="col-md-6">
                            <p class="activity-meta">
                                ...
                                <span>Kết thúc: <strong>{{ $activity->ThoiGianKetThuc->format('d/m/Y H:i') }}</strong></span>
                            </p>
                            <p class="activity-meta">
                ...
                <div class="activity-footer">
                    ...
                    @else
                    <form action="{{ route('sinhvien.dangky.drl', $activity->MaHoatDong) }}" method="POST" style="margin: 0;">
                        @csrf
        ...
        <div class="tab-pane fade" id="ctxh-content" role="tabpanel" aria-labelledby="ctxh-tab">
            @forelse($activitiesCTXH as $activity)
            @php
            $registered = $activity->dangky->count();
            $total = $activity->SoLuong;
            $percentage = $total > 0 ? ($registered / $total) * 100 : 0;
            ...
            <div class="activity-card ctxh-card">
                ...
                <div class="activity-footer">
                    ...
                    {{-- LOGIC NÚT ĐĂNG KÝ MỚI (CHO CTXH) --}}
                    @php
                    ...
                    @else
                    <form action="{{ route('sinhvien.dangky.ctxh', $activity->MaHoatDong) }}" method="POST" style="margin: 0;">
                        @csrf
