...
@section('title', 'Tất cả hoạt động')
...
<div class="container-fluid" style="max-width: 1200px; margin: 0 auto; padding: 2rem 1rem;">
    ...
    <div class="text-center">
        <div class="nav custom-tabs" role="tablist">
            <!-- Tab Tất Cả -->
            <a href="{{ route('sinhvien.news.index', ['type' => 'all']) }}" 
            ...
            <!-- Tab DRL -->
            <a href="{{ route('sinhvien.news.index', ['type' => 'drl']) }}" 
               class="nav-link {{ $currentFilter == 'drl' ? 'active' : '' }}" 
               role="tab">
            ...
            <!-- Tab CTXH -->
            <a href="{{ route('sinhvien.news.index', ['type' => 'ctxh']) }}" 
               id="ctxh-tab"
               class="nav-link {{ $currentFilter == 'ctxh' ? 'active' : '' }}" 
               role="tab">
    ...
    @else
        @foreach($thongBaos as $activity)
            {{-- Thêm class 'drl' hoặc 'ctxh' để phân biệt màu --}}
            <div class="thongbao-item {{ $activity->Loai == 'DRL' ? 'drl' : 'ctxh' }}">
                
                <div class="thongbao-header">
                    @if($activity->Loai == 'DRL')
                ...
                <p class="thongbao-date">
                    ...
                    Bắt đầu: {{ \Carbon\Carbon::parse($activity->ThoiGianBatDau)->format('H:i, d/m/Y') }}
                    <br>
                    <i class="fa fa-calendar-check me-2"></i>
                    Kết thúc: {{ \Carbon\Carbon::parse($activity->ThoiGianKetThuc)->format('H:i, d/m/Y') }}
                </p>
